...
if(class_exists('WooZone') != true) {
	class WooZone {
		...
		public function __construct($here = __FILE__)
		{
			...
			// cross sell cart - !needs to be bellow Amazon helper
			...
			if( $amz_cross_sell != false ){
				...
				if( trim($asins) != "" ){
					...
					if( count($asins) > 0 ){
						...
						$provider = 'amazon';
						$rsp = $this->get_ws_object( $provider )->api_make_request(array(
		...
		*/
		...
		public function getAmzSimilarityProducts ( $asin, $return_nr=3, $force_update=false )
		{
			...
			$similarity_respStatus = $this->get_ws_object( $provider )->is_amazon_valid_response( $similarity );
			...
			$thisProd_respStatus = $this->get_ws_object( $provider )->is_amazon_valid_response( $thisProd );
			...
			//) {
			if ( $thisProd_respStatus['status'] != 'valid' ) { // error occured!
			}
			else {
				...
				// product large image
				$retProd[$prodasin]['thumb'] = isset($thisProd['SmallImage'], $thisProd['SmallImage']['URL'])
					? $thisProd['SmallImage']['URL'] : '';
				if ( empty($retProd[$prodasin]['thumb']) ) {
								// Images
								...
								if ( isset($images['small']) && !empty($images['small']) ) {
									$retProd[$prodasin]['thumb'] = $images['small'][0];
								}
				}
			...
			//) {
			if ( $similarity_respStatus['status'] != 'valid' ) { // error occured!
			}
			else {
			...
			// similar products response is invalid
			...
			if ( $cache_request != "" && count($cache_request) > 0 ) {
				$this->db->update(
					...
					array(
						...
						'nr_products'   => $return_nr <= count($retProd) ? $return_nr : count($retProd),
					),
					array( 'ASIN' => $asin ),
					array(
						'%s',
		...
		public function update_db()  {
			...
			if (1) {
				...
				if ($this->db->get_var("show tables like '$table_name_report_assets'") == $table_name_report_assets) {
					...
					$queries = array();
					$queries['amz_assets.image_sizes'] = array(
						'main' 	=> "ALTER TABLE " . $table_name_report_assets . " %s COLUMN `image_sizes` TEXT NULL AFTER `date_download`;",
						'verify'	=> "SHOW COLUMNS FROM " . $table_name_report_assets . " LIKE 'image_sizes';",
					);
					$queries['amz_assets.download_status'] = array(
					...
					foreach ($queries as $skey => $sql) {
						...
						if ( isset($sql['verify']) ) {
							...
							if ( ! empty($status) && isset($status['Field'], $status['Type']) ) {
								...
								switch ($skey) {
									case 'amz_assets.image_sizes':
										//'image_sizes' == strtolower($status['Field'])
										if ( 'text' == strtolower( $status['Type'] ) )
											$do_main = false;
										else
											$do_main = 'modify';
										break;

									case 'amz_assets.download_status':
									//'download_status' == strtolower($status['Field'])
										if ( 'varchar(20)' == strtolower( $status['Type'] ) )
											$do_main = false;
										else
											$do_main = 'modify';
										break;
